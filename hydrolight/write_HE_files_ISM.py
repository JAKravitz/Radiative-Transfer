...
# chl
sigma,scale = lognorm_params(3,300)
chlaData = lognorm_random(sigma, scale, num)
chlaData = chlaData[chlaData<=500]

# admix
admix0 = 0
admix1 = np.arange(0,.8,.1)
admix2 = np.array([0,.1,.2,.3,.4,])
admix3 = np.array([0,.1,.2])
...
# cnap
sigma, scale = lognorm_params(50, 200)
cnapData = lognorm_random(sigma,scale, num)
cnapData = cnapData[cnapData<=1000]

# cdom
sigma, scale = lognorm_params(.7, 1.2)
acdomData = lognorm_random(sigma,scale, 10000)
acdomData = acdomData[acdomData<=4]
...
astarnap = np.linspace(0.001,.1,10) # less than normal
